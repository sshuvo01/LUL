	...
	m_Settings(settings), m_IsHDR(isHDR)
{
	...
	for (unsigned int i = 0; i < m_ColorbuffesCount; i++)
	{
		...
		GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, m_Width, 
			m_Height, 0, GL_RGBA, GL_FLOAT, NULL));
		GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (GLint)m_Settings.texFiltMode));
		GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, (GLint)m_Settings.texFiltMode));
		GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, (GLint)m_Settings.texRepMode));
		GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, (GLint)m_Settings.texRepMode));
		GLCALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, 
			m_ColorbuffersID[i], 0));
	} // end of for
